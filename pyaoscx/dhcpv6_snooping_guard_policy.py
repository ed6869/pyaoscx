...
class Dhcpv6SnoopingGuardPolicy(PyaoscxModule):
    """
    Provide configuration management for DHCPv6 snooping guard policies on
        AOS-CX devices. Policies live under system/dhcpv6_snooping_guard_policies
        and are indexed by name.
    """

    base_uri = "system/dhcpv6_snooping_guard_policies"
    resource_uri_name = "dhcpv6_snooping_guard_policies"
    ...
    def from_uri(cls, session, uri):
        """
        Create a Dhcpv6SnoopingGuardPolicy object given a URI.
        ...
        index_pattern = re.compile(
            r"(.*)dhcpv6_snooping_guard_policies/(?P<index>.+)"
        )
    ...
    def __str__(self):
        return "DHCPv6 Snooping Guard Policy {0}".format(self.name)
