...
class Ipv6DestinationGuardPolicy(PyaoscxModule):
    """
    Provide configuration management for IPv6 destination guard policies on
        AOS-CX devices. Policies live under system/ipv6_destination_guard_policies
        and are indexed by name.
    """

    base_uri = "system/ipv6_destination_guard_policies"
    resource_uri_name = "ipv6_destination_guard_policies"
    ...
    def get(self, depth=None, selector=None):
        """
        Perform a GET call to retrieve data for a policy and fill the object
    ...
    def get_all(cls, session):
        """
        Perform a GET call to retrieve all policies, and create a dictionary
            containing them.

        :param cls: Object's class.
        :param session: pyaoscx.Session object.
        :return: Dictionary containing the name as keys and the policy objects
            as values.
        """
        ...
        for uri in uri_list:
            index, policy = cls.from_uri(session, uri)
    ...
    def update(self):
        """
        Perform a PUT call to apply changes to an existing policy.
    ...
    def create(self):
        """
        Perform a POST call to create a new policy. Only returns if no
            exception is raised.
    ...
    def delete(self):
        """
        Perform DELETE call to delete a policy.
        """
        try:
    ...
    def from_uri(cls, session, uri):
        """
        Create a Ipv6DestinationGuardPolicy object given a URI.

        :param cls: Class calling the method.
        :param session: pyaoscx.Session object.
        :param uri: a String with a URI.
        :return: tuple containing both the name and the policy object.
        """
        index_pattern = re.compile(
            r"(.*)ipv6_destination_guard_policies/(?P<index>.+)"
        )
        index = index_pattern.match(uri).group("index")
        policy = cls(session, index)
        return index, policy

    def __str__(self):
        return "IPv6 Destination Guard Policy {0}".format(self.name)

    @PyaoscxModule.deprecated
    def get_uri(self):
        """
        Method used to obtain the specific policy URI.
