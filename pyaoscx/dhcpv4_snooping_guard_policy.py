...
class Dhcpv4SnoopingGuardPolicy(PyaoscxModule):
    """
    Provide configuration management for DHCPv4 snooping guard policies on
        AOS-CX devices. Policies live under system/dhcpv4_snooping_guard_policies
        and are indexed by name.
    """

    base_uri = "system/dhcpv4_snooping_guard_policies"
    resource_uri_name = "dhcpv4_snooping_guard_policies"
    ...
    def from_uri(cls, session, uri):
        """
        Create a Dhcpv4SnoopingGuardPolicy object given a URI.
        ...
        index_pattern = re.compile(
            r"(.*)dhcpv4_snooping_guard_policies/(?P<index>.+)"
        )
    ...
    def __str__(self):
        return "DHCPv4 Snooping Guard Policy {0}".format(self.name)
